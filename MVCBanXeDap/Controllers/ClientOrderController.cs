...
namespace MVCBanXeDap.Controllers
{
    public class ClientOrderController : Controller
    {
        ...
        public ClientOrderController(IjwtToken jwtToken)
        {
        ...
        public async Task<IActionResult> Detail(string idOrder)
        {
            ...
            HttpResponseMessage httpResponseMessage = await _client.GetAsync(_client.BaseAddress + $"ClientOrder/GetInvoiceData/{idOrder}");
        ...
        public async Task<IActionResult> TakeInvoice(string maHoaDon)
        {
            ...
            SetAuthorizationHeader();
            // Gọi API để lấy thông tin hóa đơn
            HttpResponseMessage httpResponseMessage = await _client.GetAsync(_client.BaseAddress + $"ClientOrder/GetInvoiceData/{maHoaDon}");
            if (httpResponseMessage.IsSuccessStatusCode)
            {
                string data = await httpResponseMessage.Content.ReadAsStringAsync();
                invoice = JsonConvert.DeserializeObject<InvoiceVM>(data);
        ...
        public async Task<IActionResult> ExportAllInvoicesAsSinglePDF()
        {
            ...
            SetAuthorizationHeader();
            HttpResponseMessage httpResponseMessage = await _client.GetAsync(_client.BaseAddress + $"ClientOrder/GetAllInvoiceData/");
            if (httpResponseMessage.IsSuccessStatusCode)
            {
                string data = await httpResponseMessage.Content.ReadAsStringAsync();
                ...
                [FromQuery] string? tinhTrang)
        {
            ...
            SetAuthorizationHeader();
            HttpResponseMessage httpResponseMessage = await _client.GetAsync(_client.BaseAddress + "ClientOrder/get");
            if (httpResponseMessage.IsSuccessStatusCode)
            {
                string data = await httpResponseMessage.Content.ReadAsStringAsync();
